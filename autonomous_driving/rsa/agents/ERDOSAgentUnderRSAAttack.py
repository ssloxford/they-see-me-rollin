...
pattern_path = os.getenv("RSA_PATTERN")
raw_pattern = io.imread(pattern_path)
pattern_BGR = cv2.cvtColor(raw_pattern, cv2.COLOR_RGB2BGR)
    ...
    def run_step(self, input_data, timestamp):
        ...
        for key, val in input_data.items():
            ...
            if key in self._camera_streams:
                # Check if the current stream is from the center camera
